...
public class Mano {
    ...
    public void calculaJugada() {
        this.verbose = "La mano es: ";
        for (Carta carta : cartas) {
            this.verbose += carta.toString() + ", ";
        }
        this.verbose += "\n- Mejor Jugada: ";
    ...
    public boolean esColor(boolean comprobado) {
        ...
        for (int i = 0; i < arrayPalos.length; i++) {
            if (arrayPalos[i] >= 5) {
                ...
                hayColor = true;
            } else if (arrayPalos[i] == 4 && comprobado) {
    ...
    public boolean esEscalera() {
        ...
        for (int i = aux; i >= 1; i--) {
            if ((arrayNums[i] >= 1) && arrayNums[i] < 4) {
                ...
                if (arrayNums[i - 1] >= 1 && arrayNums[i - 1] < 4) {
                    ...
                    if (numLinked == 3) {
                ...
                } else if ((//primero comprobamos el gutshot de una sola y hueco
                        numLinked == 0
                        && arrayNums[i - 2] >= 1
                        && arrayNums[i - 3] >= 1
                        && arrayNums[i - 4] >= 1)
                        || (//comprobamos el gutshot de dos hueco y dos
                        numLinked == 1
                        && arrayNums[i - 2] >= 1
                        && arrayNums[i - 3] >= 1)
                        || (//comprobamos el gutshot de dos hueco y dos
                        numLinked == 2
                        && arrayNums[i - 2] >= 1)) {
                    if (!this.draws.contains(Jugadas.escalera)) {
                        this.draws.add(Jugadas.escalera);
                        this.gutshot = true;
                    }
                }
            }
